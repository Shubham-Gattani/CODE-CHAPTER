...
Such as Regn details, fee payment, progress, exam centre , hall ticket, results etc. BUT HOW DO I EMBED THIS IN THE CURRENT DATABASE STRUCTURE? DO WE NEED TO ADD A NEW TABLE LIKE STUDENT_DASHBOARD?
...
5. Docstrings: Add docstrings to models for clarity.
6. Consistency: Use either all backref or all back_populates for relationships.
"""
...
class Student(db.Model):
    ...
    name = db.Column(db.String(100), nullable=False)
    personal_email = db.Column(db.String(255), unique=True, nullable=True) # personal or official ?
    ...
    quizzes = db.relationship('Quiz', back_populates='student', lazy=True) # each student can have many quizzes
...
class CoreCourse(db.Model): #  Do we have to add PROJECT COURSES also? CAn add
    """
...
class CollegeCourse(db.Model): # This is a complex code. We will simplify if if needed while making the APIs.
    ...
    quizzes = db.relationship('Quiz', back_populates='college_course', lazy=True)
    enrollments = db.relationship('StudentCourseEnrollment', back_populates='college_course', lazy=True)

class Quiz(db.Model):
    """
    Represents a student's marks for a specific quiz or assessment in a particular course, college, year, and term.
    Each record corresponds to one student's performance in one instance of a course offering. A Student can have multiple Quizzes; each Quiz belongs to one Student.
    """
    __tablename__ = 'quizzes'
    # Foreign keys to CollegeCourse (composite)
    ...
    # NOTE: You use composite primary keys and composite foreign keys correctly, but ensure your ORM queries always provide all key fields.

    # Composite foreign key to CollegeCourse
    __table_args__ = (
        db.ForeignKeyConstraint(
    ...
    # ASK: DO WE HAVE TO ADD viva1, viva2, final_interview etc. also?
    ...
    college_course = db.relationship('CollegeCourse', primaryjoin="and_(Quiz.course_code==CollegeCourse.course_code, Quiz.college_id==CollegeCourse.college_id, Quiz.year==CollegeCourse.year, Quiz.term==CollegeCourse.term)")
    student = db.relationship('Student', back_populates='quizzes')

class Role(db.Model, RoleMixin): # minimal Role model to avoid any errors related to flask-security.
    id = db.Column(db.Integer, primary_key=True)
